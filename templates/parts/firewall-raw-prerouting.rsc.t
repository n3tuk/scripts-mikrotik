...
add chain="$runId:prerouting" \
    action=jump jump-target="$runId:check:bogons" \
    comment="Process packets for bogus network addresses"
add chain="$runId:prerouting" \
    protocol=tcp \
    action=jump jump-target="$runId:check:tcp" \
    comment="Check TCP packets"
add chain="$runId:prerouting" \
    protocol=udp \
    action=jump jump-target="$runId:check:udp" \
...
add chain="prerouting" \
    action=jump jump-target="$runId:prerouting" \
